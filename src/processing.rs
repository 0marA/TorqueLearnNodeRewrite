...
pub fn process() {
    ...
    WalkDir::new("deploy/Tutorials")
        // make it iterable
        .into_iter()
        // skip going into non directories and directories without an index.html
        ...
        .for_each(|x|{
            match x {
                Ok(dir) => {
                    // declare String variable "let"
                    let mut ret : String = String::new();
                    // declare var p that holds the directory name minus deploy at the beginning
                    let p : &str = dir.path().to_str().expect("Failed path->str").trim_start_matches("deploy");

                    // declare variable "paths" that holds: the directory path, expect error with unwrap, with for each item put it into it
                    ...
                    for path in paths {
                        ...
                        // add TOP_DIR_ENTRY to ret
                        ret.push_str(TOP_DIR_ENTRY);
                        // then name
                        ret.push_str(name);
                        // then MIDDLE_SECTION_ENTRY
                        ret.push_str(MIDDLE_SECTION_ENTRY);
                        
                        // if path + index.html exists, basically if there's an index.html in that path,
